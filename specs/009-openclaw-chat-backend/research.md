...
- **No persisted map needed — continuity is automatic**: Confirmed in `resolveSessionStoreKey` (`/app/dist/store-jR_nVe0n.js:71`): the gateway canonicalizes whatever `sessionKey` it receives — a plain string `demo-1` becomes `agent:main:demo-1` via `canonicalizeSessionKeyForAgent`, an already-`agent:…` key is kept, and `main` resolves to the main session. OpenClaw **persists** each canonical key on disk (`~/.openclaw/agents/main/sessions/sessions.json`). Therefore the `sessionId → sessionKey` transform is a **pure deterministic function**, not a stored lookup table. Sending the **same `sessionId`** again yields the same canonical key → OpenClaw reloads the same history → the conversation continues. A **different/new `sessionId`** starts a fresh conversation. First send with a new key auto-creates the session (load-then-persist); the frontend does NOT need to call `sessions.create`. The frontend is responsible for keeping a stable `sessionId` per conversation (e.g. persisted client-side or derived from a user id).
- **Alternatives considered**: Global single session (rejected — violates SC-005 isolation); backend-generated/persisted session map (rejected — that would be a SessionStore, violating Principle I; and it is unnecessary because the mapping is deterministic and OpenClaw already persists it).
