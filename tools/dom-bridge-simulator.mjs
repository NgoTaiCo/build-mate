...
<section data-build-category="cpu"><strong>CPU</strong><span data-selected-name>Chưa chọn</span><button data-build-action="open-category">Chọn</button><button data-build-action="decrease-quantity" aria-label="Giảm số lượng">-</button><input data-build-action="quantity-input" type="number" min="1" value="1" readonly><button data-build-action="increase-quantity" aria-label="Tăng số lượng">+</button><button data-build-action="remove-component">Xóa</button></section>
<section data-build-category="gpu"><strong>VGA</strong><span data-selected-name>Chưa chọn</span><button data-build-action="open-category">Chọn</button><button data-build-action="decrease-quantity" aria-label="Giảm số lượng">-</button><input data-build-action="quantity-input" type="number" min="1" value="1" readonly><button data-build-action="increase-quantity" aria-label="Tăng số lượng">+</button><button data-build-action="remove-component">Xóa</button></section>
...
function refreshTotal(){const total=[...document.querySelectorAll('[data-build-category]')].reduce((sum,row)=>sum+Number(row.dataset.price||0)*Number(row.dataset.quantity||1),0);document.querySelector('#build-total').textContent='Tổng: '+total+' VND';}
function refreshQuantity(row){row.querySelector('[data-build-action="quantity-input"]').value=row.dataset.quantity||1;refreshTotal();}
let currentCategory=null; const modal=document.querySelector('[data-product-modal]');
document.addEventListener('click',(event)=>{const button=event.target.closest('button');if(!button)return;if(button.dataset.buildAction==='open-category'){const row=button.closest('[data-build-category]');currentCategory=row.dataset.buildCategory;document.querySelector('#modal-title').textContent='Chọn '+currentCategory;document.querySelector('#products').innerHTML=catalog[currentCategory].map(p=>'<article class="product" data-vendor-product-id="'+p.id+'" data-sku="'+p.sku+'"><strong>'+p.name+'</strong><button data-build-action="select-product">Chọn</button></article>').join('');modal.showModal();}if(button.dataset.buildAction==='close-modal')modal.close();if(button.dataset.buildAction==='select-product'){const card=button.closest('[data-vendor-product-id]');const item=catalog[currentCategory].find(p=>p.id===card.dataset.vendorProductId);const row=document.querySelector('[data-build-category="'+currentCategory+'"]');row.dataset.vendorProductId=item.id;row.dataset.sku=item.sku;row.dataset.selectedName=item.name;row.dataset.price=item.price;row.dataset.quantity=1;row.querySelector('[data-selected-name]').textContent=item.name;refreshQuantity(row);modal.close();}if(button.dataset.buildAction==='increase-quantity'||button.dataset.buildAction==='decrease-quantity'){const row=button.closest('[data-build-category]');if(!row.dataset.vendorProductId)return;row.dataset.quantity=Math.max(1,Number(row.dataset.quantity||1)+(button.dataset.buildAction==='increase-quantity'?1:-1));refreshQuantity(row);}if(button.dataset.buildAction==='remove-component'){const row=button.closest('[data-build-category]');delete row.dataset.vendorProductId;delete row.dataset.sku;delete row.dataset.selectedName;delete row.dataset.price;delete row.dataset.quantity;row.querySelector('[data-selected-name]').textContent='Chưa chọn';refreshQuantity(row);}});
</script></body></html>`;
